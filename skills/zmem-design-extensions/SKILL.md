...
description: Design, implement, and verify zmem Python expanders and hooks using ExpansionContext, HookRegistry, extension discovery, trust, extend, and overwrite contracts. Use when an agent needs a new annotation type, context action, relationship, diagnostic, read-only post-expansion observer, or a controlled override of built-in behavior.
---
...
- Use an **expander** when an annotation must add an entry or relationship, decay or cancel a target, or emit a canonical diagnostic.
...
1. State the user-visible behavior, annotation grammar, stable uppercase extension ID, accepted inputs, score rules, diagnostics, and failure behavior.
...
9. Exercise a sample commit and inspect resulting entries, effects, relationships, and diagnostics.
